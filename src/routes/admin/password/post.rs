use axum::{Form, response::{IntoResponse, Redirect}, extract::State, Extension};
...
pub async fn change_password(
    ...
    if form.new_password.expose_secret() != form.new_password_check.expose_secret() {
        return Ok((signed_jar.add(Cookie::new(
                "_flash",
    ...
    let username = get_username(user_id, &pool).await.map_err(|e| AdminDashboardError::UnexpectedError(e.into()))?;
    ...
    match crate::authentication::change_password(user_id, form.new_password, &pool)
        .await {
            Ok(_) => Ok((
                    signed_jar.add(Cookie::new(
                        "_flash",
